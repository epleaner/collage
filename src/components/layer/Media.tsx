...
const Media = ({ layer, style }: MediaProps) => {
  ...
  }, [layer.startTime, layer.endTime, layer.loopMode, layer.playing, isReversed, videoDuration]);
  ...
  useEffect(() => {
    ...
    const handleMetadataLoaded = () => {
      if (video.duration && !isNaN(video.duration)) {
        ...
        // Store video element reference in layer for external access
        updateLayer(layer.id, { videoElement: video });
  ...
  }, [mediaType, layer.id, layer.startTime, layer.endTime, initialTime, updateLayer, attemptPlay]);
  ...
  useEffect(() => {
    if (mediaType === 'video' && videoRef.current) {
      updateLayer(layer.id, { videoElement: videoRef.current });
    }

    // Cleanup: remove video element reference when component unmounts or video changes
    return () => {
      if (mediaType === 'video') {
        updateLayer(layer.id, { videoElement: null });
      }
    };
  }, [mediaType, layer.id, updateLayer]);
  ...
  useEffect(() => {
    ...
    console.log(
      `Loop settings updated for layer ${layer.id}: ${startTime}s - ${endTime}s, mode: ${layer.loopMode}`
    );
    ...
    if (startTimeChanged || video.currentTime < startTime || video.currentTime > endTime) {
      console.log(
        `Repositioning video to start time ${startTime}s (was at ${video.currentTime.toFixed(2)}s)`
      );
      video.currentTime = startTime;
