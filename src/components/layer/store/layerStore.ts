...
const createBaseLayer = (): Layer => ({
  id: 'base-image',
  srcUrl:
    'https://archive.org/download/jojo-no-kimyou-na-bouken-1993-all-episodes-english-dubbed/jojos-bizarre-adventure-dub-episode-2.mp4',
  transform: {
    position: { x: 0, y: 0 },
    scale: 1,
    rotation: 0,
...
const createPatternLayer = (): Layer => ({
  id: 'pattern-layer',
  srcUrl:
    'https://archive.org/download/x-men-marvel-anime-1080p-ai-upscale/X-Men%20Anime/X-Men%20%28Marvel%20ANIME%29%20-%20Episode%2002%20-%20U-Men...Mutant%20Hunting.mp4',
  transform: {
    position: { x: 0, y: 0 },
    scale: 1,
    rotation: 0,
...
export const useLayerStore = create<LayerState>((set) => ({
  ...
  setLayerSrcUrl: (layerId, srcUrl) => {
    set((state) => ({
      layers: state.layers.map((layer) =>
        layer.id === layerId ? ({ ...layer, srcUrl } as Layer) : layer
      ),
    }));
  },
  setLayerTimeRange: (layerId, startTime, endTime) => {
    set((state) => ({
      layers: state.layers.map((layer) =>
        layer.id === layerId ? ({ ...layer, startTime, endTime } as Layer) : layer
      ),
    }));
  },
  setLayerLoopMode: (layerId, loopMode) => {
    set((state) => ({
      layers: state.layers.map((layer) =>
        layer.id === layerId ? ({ ...layer, loopMode } as Layer) : layer
      ),
    }));
  },
  ...
  setLayerTextContent: (layerId, textContent) => {
    set((state) => ({
      layers: state.layers.map((layer) =>
        layer.id === layerId ? ({ ...layer, textContent } as Layer) : layer
      ),
    }));
  },
